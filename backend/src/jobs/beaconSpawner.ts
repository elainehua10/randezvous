...
import { locationBroker } from "../web-socket-server/ws";
...
async function getRandomCoordinates(groupId: string) {
  try {
    ...
    const jitter = () => (Math.random() - 0.5) * 0.02; // ~0.002 deg = ~222m
    return {
...
async function notifyUnreachedUsers(groupId: string) {
  try {
    ...
    if (unreached.length > 0) {
      ...
      console.log(
        "✅ All users have reached the beacon or notifications are disabled."
      );
    }
  } catch (err) {
